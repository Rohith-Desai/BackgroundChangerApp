...
public class CityFragmentNew  extends Fragment {
    ...
    String img1, img2;
  public  static   int keycountcitynew = 0;
    RecyclerView neonCateRecNew;

    CityAdapterListenernew cityAdapterListenernew;
    public interface CityAdapterListenernew {
        void onCityItemClickednew(String back, String forei,int position);
    }
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ...
        if (RemoteConfigValues.getOurRemote().getCityUrl()!=null){
            if (!RemoteConfigValues.getOurRemote().getCityUrl().equals("")){
                if (RemoteConfigValues.getOurRemote().getEnableRewardAfter()!=null){
                    if (!RemoteConfigValues.getOurRemote().getEnableRewardAfter().equals("")){
                        for (int i=1;i<=Integer.parseInt(RemoteConfigValues.getOurRemote().getEnableRewardAfter());i++){
                            beforeKeythumb.add(RemoteConfigValues.getOurRemote().getCityUrl()+"city_thumb_"+i+".jpg");
                            beforeKeyback.add(RemoteConfigValues.getOurRemote().getCityUrl()+"city_back_"+i+".png");
                            beforeKeyfore.add(RemoteConfigValues.getOurRemote().getCityUrl()+"city_fore_"+i+".png");
                        }
        ...
        if (RemoteConfigValues.getOurRemote().getCityUrl()!=null){
            if (!RemoteConfigValues.getOurRemote().getCityUrl().equals("")){
                if (RemoteConfigValues.getOurRemote().getCityCount()!=null){
                    if (!RemoteConfigValues.getOurRemote().getCityCount().equals("")){
                        if (RemoteConfigValues.getOurRemote().getEnableRewardAfter()!=null){
                            if (!RemoteConfigValues.getOurRemote().getEnableRewardAfter().equals("")){
                                for (int i=Integer.parseInt(RemoteConfigValues.getOurRemote().getEnableRewardAfter())+1;i<=Integer.parseInt(RemoteConfigValues.getOurRemote().getCityCount());i++){
                                    neonThumb.add(RemoteConfigValues.getOurRemote().getCityUrl()+"city_thumb_"+i+".jpg");
                                    neonBack.add(RemoteConfigValues.getOurRemote().getCityUrl()+"city_back_"+i+".png");
                                    neonFore.add(RemoteConfigValues.getOurRemote().getCityUrl()+"city_fore_"+i+".png");
                                }
                            }
                        }
                    }
...
//to check and seperate the frames that are downloaded in natureframes list before  key list
        ...
        for (int i = 0; i < neonThumb.size(); i++) {
            ...
            if (img1.contains("https")) {
                ...
                if (backmediaStorageDir.exists() && foremediaStorageDir.exists()) {
                    ...
                    keycountcitynew = downloadedThumb.size();
                    System.out.println(keycountcitynew);
                } else {
                    //adding the not downloaded frames
    ...
    private void classAndWidgetIntialize() {
        ...
        CityframesAdapterNew cityframesAdapterNew = new CityframesAdapterNew(getActivity(), beforeKeydownloadthumb,beforeKeydownloadback,beforeKeydownloadfore);
        ...
        neonCateRecNew.setAdapter(cityframesAdapterNew);


    }
    public class CityframesAdapterNew  extends RecyclerView.Adapter<CityframesAdapterNew.ViewHolder> {
        ...
        public CityframesAdapterNew(Activity activity, ArrayList<String> neonThumb, ArrayList<String> neonBack, ArrayList<String> neonFore){
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, @SuppressLint("RecyclerView") int position) {
            ...
            int newkeycount;
            if (keycountcitynew > 0) {
                newkeycount = oldkeycount + keycountcitynew;
...
//                        if (position>=Integer.parseInt(RemoteConfigValues.getOurRemote().getEnableRewardAfter())){
            ...
            holder.neonnewImg.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if(cityAdapterListenernew != null) {
                        cityAdapterListenernew.onCityItemClickednew(neonBack.get(position),neonFore.get(position),position);
                    }
    ...
    public void onAttach(Context context) {
        ...
        try {
            cityAdapterListenernew = (CityAdapterListenernew) context;
