...
public class AnniversaryFragmentNew extends Fragment {
    ...
    String img1, img2;
    public  static  int keycountnewanniversary = 0;
    RecyclerView neonCateRecNew;

    AnniversaryAdapterListenernew anniversaryAdapterListenernew;
    public interface AnniversaryAdapterListenernew {
        void onAnniversaryItemClickednew(String back, String forei,int position);
    }
                        ...
                        if (RemoteConfigValues.getOurRemote().getEnableRewardAfter()!=null){
                            if (!RemoteConfigValues.getOurRemote().getEnableRewardAfter().equals("")){
                                ...
                                for (int i=Integer.parseInt(RemoteConfigValues.getOurRemote().getEnableRewardAfter())+1;i<=Integer.parseInt(RemoteConfigValues.getOurRemote().getAnniversaryCount());i++){
                                    neonThumb.add(RemoteConfigValues.getOurRemote().getAnniversaryUrl()+"anniversary_thumb_"+i+".jpg");
                                    neonBack.add(RemoteConfigValues.getOurRemote().getAnniversaryUrl()+"anniversary_back_"+i+".png");
                                    neonFore.add(RemoteConfigValues.getOurRemote().getAnniversaryUrl()+"anniversary_fore_"+i+".png");
                                }
...
//to check and seperate the frames that are downloaded in natureframes list before  key list
        ...
        for (int i = 0; i < neonThumb.size(); i++) {
            ...
            if (img1.contains("https")) {
                ...
                if (backmediaStorageDir.exists() && foremediaStorageDir.exists()) {
                    ...
                    keycountnewanniversary = downloadedThumb.size();
                    System.out.println(keycountnewanniversary);
                } else {
                    //adding the not downloaded frames
    ...
    private void classAndWidgetIntialize() {
        ...
        AnniversaryframesAdapterNew anniversaryframesAdapterNew = new AnniversaryframesAdapterNew(getActivity(), beforeKeydownloadthumb,beforeKeydownloadback,beforeKeydownloadfore);
        ...
        neonCateRecNew.setAdapter(anniversaryframesAdapterNew);


    }
    public class AnniversaryframesAdapterNew  extends RecyclerView.Adapter<AnniversaryframesAdapterNew.ViewHolder> {
        ...
        public AnniversaryframesAdapterNew(Activity activity, ArrayList<String> neonThumb, ArrayList<String> neonBack, ArrayList<String> neonFore){
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
            ...
            int newkeycount;
            if (keycountnewanniversary > 0) {
                newkeycount = oldkeycount + keycountnewanniversary;
...
//                        if (position>=Integer.parseInt(RemoteConfigValues.getOurRemote().getEnableRewardAfter())){
            ...
            holder.neonnewImg.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if(anniversaryAdapterListenernew != null) {
                        anniversaryAdapterListenernew.onAnniversaryItemClickednew(neonBack.get(position),neonFore.get(position),position);
                    }
    ...
    public void onAttach(Context context) {
        ...
        try {
            anniversaryAdapterListenernew = (AnniversaryAdapterListenernew) context;
