...
public class AnimalFragmentNew extends Fragment {
    ...
    String img1, img2;
   public static int keycountanimalnew = 0;
    RecyclerView neonCateRecNew;

    AnimalAdapterListenernew animalAdapterListenernew;
    public interface AnimalAdapterListenernew {
        void onAnimalItemClickednew(String back, String forei,int position);
    }
    @Override
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        ...
        if (RemoteConfigValues.getOurRemote().getAnimalUrl()!=null){
            if (!RemoteConfigValues.getOurRemote().getAnimalUrl().equals("")){
                if (RemoteConfigValues.getOurRemote().getEnableRewardAfter()!=null){
                    if (!RemoteConfigValues.getOurRemote().getEnableRewardAfter().equals("")){
                        for (int i=1;i<=Integer.parseInt(RemoteConfigValues.getOurRemote().getEnableRewardAfter());i++){
                            beforeKeythumb.add(RemoteConfigValues.getOurRemote().getAnimalUrl()+"animal_thumb_"+i+".jpg");
                            beforeKeyback.add(RemoteConfigValues.getOurRemote().getAnimalUrl()+"animal_back_"+i+".png");
                            beforeKeyfore.add(RemoteConfigValues.getOurRemote().getAnimalUrl()+"animal_fore_"+i+".png");
                        }
        ...
        if (RemoteConfigValues.getOurRemote().getAnimalUrl()!=null){
            if (!RemoteConfigValues.getOurRemote().getAnimalUrl().equals("")){
                if (RemoteConfigValues.getOurRemote().getAnimalCount()!=null){
                    if (!RemoteConfigValues.getOurRemote().getAnimalCount().equals("")){
                        if (RemoteConfigValues.getOurRemote().getEnableRewardAfter()!=null){
                            if (!RemoteConfigValues.getOurRemote().getEnableRewardAfter().equals("")){
                                for (int i=Integer.parseInt(RemoteConfigValues.getOurRemote().getEnableRewardAfter())+1;i<=Integer.parseInt(RemoteConfigValues.getOurRemote().getAnimalCount());i++){
                                    neonThumb.add(RemoteConfigValues.getOurRemote().getAnimalUrl()+"animal_thumb_"+i+".jpg");
                                    neonBack.add(RemoteConfigValues.getOurRemote().getAnimalUrl()+"animal_back_"+i+".png");
                                    neonFore.add(RemoteConfigValues.getOurRemote().getAnimalUrl()+"animal_fore_"+i+".png");
                                }
                            }
                        }
                    }
...
//to check and seperate the frames that are downloaded in natureframes list before  key list
        ...
        for (int i = 0; i < neonThumb.size(); i++) {
            ...
            if (img1.contains("https")) {
                ...
                if (backmediaStorageDir.exists() && foremediaStorageDir.exists()) {
                    ...
                    keycountanimalnew = downloadedThumb.size();
                    System.out.println(keycountanimalnew);
                } else {
                    //adding the not downloaded frames
    ...
    private void classAndWidgetIntialize() {
        ...
        AnimalframesAdapterNew animalframesAdapterNew = new AnimalframesAdapterNew(getActivity(), beforeKeydownloadthumb,beforeKeydownloadback,beforeKeydownloadfore);
        ...
        neonCateRecNew.setAdapter(animalframesAdapterNew);


    }
    public class AnimalframesAdapterNew  extends RecyclerView.Adapter<AnimalframesAdapterNew.ViewHolder> {
        ...
        public AnimalframesAdapterNew(Activity activity, ArrayList<String> neonThumb, ArrayList<String> neonBack, ArrayList<String> neonFore){
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, @SuppressLint("RecyclerView") int position) {
            ...
            int newkeycount;
            if (keycountanimalnew > 0) {
                newkeycount = oldkeycount + keycountanimalnew;
...
//                        if (position>=Integer.parseInt(RemoteConfigValues.getOurRemote().getEnableRewardAfter())){
            ...
            holder.neonnewImg.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if(animalAdapterListenernew != null) {
                        animalAdapterListenernew.onAnimalItemClickednew(neonBack.get(position),neonFore.get(position),position);
                    }
    ...
    public void onAttach(Context context) {
        ...
        try {
            animalAdapterListenernew = (AnimalAdapterListenernew) context;
